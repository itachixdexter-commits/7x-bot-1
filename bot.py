...
**الرد المطلوب:**
- إذا كان الكود صحيح: "✅ كود صحيح مايحتاج تعديل"
- إذا كان هناك أخطاء: اشرح الأخطاء ثم أرسل الكود المصحح
"""
        ...
        fixed_code = result['candidates'][0]['content']['parts'][0]['text'].strip()
        
        await processing_msg.delete()
        
        if "✅ كود صحيح مايحتاج تعديل" in fixed_code:
            result_text = f"""
✅ **تحليل الكود**
...
📌 **ملاحظة:** تم تصحيح الأخطاء الموجودة في الكود.
"""
        ...
        if len(result_text) > 4096:
            ...
            for part in parts:
                await update.message.reply_text(
                    ...
                    parse_mode='Markdown',
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔄 تصحيح كود آخر", callback_data='fix_code')],
                        [InlineKeyboardButton("العودة للقائمة الرئيسية 🔙", callback_data='main_menu')]
                    ])
                )
        else:
        ...
        await update.message.reply_text(
            f"❌ حدث خطأ في الاتصال بـ Gemini: {e}\nتأكد من صحة المفتاح وحاول مرة أخرى.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔄 إعادة المحاولة", callback_data='fix_code')],
                [InlineKeyboardButton("العودة للقائمة الرئيسية 🔙", callback_data='main_menu')]
            ])
...
async def button(update: Update, context) -> int:
    ...
    elif query.data == 'fix_code':
        return await fix_code(update, context)
...
async def get_phone_analysis(update: Update, context) -> int:
    ...
    try:
        ...
        if NUMVERIFY_API_KEY != "8d6ce257e7b6502633705e2330fd0439":
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            GETTING_CODE_TO_FIX: [MessageHandler(filters.TEXT & ~filters.COMMAND, get_fixed_code)],
        },
