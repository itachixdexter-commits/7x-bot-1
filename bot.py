...
# set higher logging level for httpx to avoid getting too much log output
...
async def handle_message(update: Update, context) -> None:
    if update.message.text:
        ...
        # Check if the user is responding to a specific prompt
        # For example, if the last message from the bot was asking for a URL for website files
        # This part will need more sophisticated state management for a real bot
        # For now, we'll just echo the message back if it's not a command or callback
        await update.message.reply_text("تلقيت رسالتك: " + text)

async def get_website_html(update: Update, context) -> None:
    ...
    try:
        ...
        file_name = f"website_{re.sub(r'[^a-zA-Z0-9]', '_', url)}.html"
        ...
        await update.message.reply_text(f"حدث خطأ غير متوقع: {e}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("العودة للقائمة الرئيسية 🔙", callback_data='main_menu')]]))

async def get_ip_info(update: Update, context) -> None:
    ...
    try:
        ...
        if data.get("status") == "success":
            info_text = f"🌐 الدومين: {query_text if not re.match(r'^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$', query_text) else 'غير متوفر'}\n"
...
async def generate_temp_email(update: Update, context) -> None:
    try:
        ...
        await update.callback_query.edit_message_text(f"حدث خطأ غير متوقع: {e}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("العودة للقائمة الرئيسية 🔙", callback_data='main_menu')]]))
...
async def send_tv_channels_for_country(update: Update, context, country_code: str) -> None:
    try:
        ...
        channels_text = "قنوات تلفزيونية لدولة "
        ...
        channels_text += countries.get(country_code, "غير معروفة") + ":\n\n"
...
async def search_telegram_user(update: Update, context) -> None:
    ...
    try:
        # Telegram user IDs are numeric, usernames start with @
        if user_id_or_username.isdigit():
            # It's a user ID, construct a direct link to the user's profile
            ...
            # It's a username, construct a direct link to the user's profile
...
async def generate_joke(update: Update, context) -> None:
    jokes = [
        ...
        "ماذا قال الجبل للجبل؟ لم نلتقِ منذ زمن طويل.",
        "لماذا سميت حاسة الشم بهذا الاسم؟ لأنها تشم.",
        "ما هو وجه الشبه بين الكمبيوتر والحفرة؟ كلاهما يحتاج إلى حفر.",
...
async def check_link(update: Update, context) -> None:
    url = update.message.text
    if not url or not (url.startswith('http://') or url.startswith('https://')):
        await update.message.reply_text("الرجاء إرسال رابط صحيح يبدأ بـ http:// أو https://", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("العودة للقائمة الرئيسية 🔙", callback_data='main_menu')]]))
        return
...
async def get_roblox_account_info(update: Update, context) -> None:
...
async def decorate_text(update: Update, context) -> None:
...
async def decrypt_roblox_script(update: Update, context) -> None:
    if update.message.document:
        ...
        try:
            ...
            await update.message.reply_text(f"حدث خطأ أثناء معالجة الملف: {e}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("العودة للقائمة الرئيسية 🔙", callback_data='main_menu')]]))
    else:
...
def main() -> None:
    ...
    # Message handlers for specific features that require text input after a button press
    # This requires state management, for now, we'll use a simple approach
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.Regex(r'^(http|https)://'), get_website_html))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.Regex(r'^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$|^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$'), get_ip_info))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.Regex(r'^@?[a-zA-Z0-9_]+$'), search_telegram_user))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message)) # Generic text handler
    application.add_handler(MessageHandler(filters.Document.ALL, decrypt_roblox_script))
